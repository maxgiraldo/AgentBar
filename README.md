...
- generates `~/Library/LaunchAgents/com.max.agentbar.plist` for the current user
...
The LaunchAgent plist is generated by `scripts/install.sh` at install time so it can include the current user's home directory and install paths. Do not commit a generated `com.max.agentbar.plist`.
